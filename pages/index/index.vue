	...
	<view>
		...
		<view class="cu-form-group margin-top news" >
			...
			<view class="item_list" style="margin-left: 0;">
				<navigator v-for=" (item) in newList" :url="'./news_detail/news_detail?nid='+item.id" :key="item.id" class="item" >通知：{{item.title}}</navigator>
			</view>
		</view>
		<!-- 工作区 -->
		<view class="cu-form-group margin-top work">
			...
			<view class="work_content">
				<view v-for="(item) in workContentList" class="work_item" :key="item.id" @click="toPage(item.id)">
		...
		<view class="cu-form-group margin-top active">
			...
			<view class="active">
				<view class="activity_card" v-for="(item,index) in hotActivityList" :key="index" @click="goDetail(item.id)">
					...
					<view style="margin-top: -190rpx;">
						<view class='sellout' :style="{'border-color':(item.status== 0 ?'transparent transparent #F59A23' : item.status==1 ? 'transparent transparent #4C96CB':'transparent transparent #AAAAAA' )}">
							...
							<template v-elseif="item.status==1">已参加</template>
							<template v-elseif="item.status==2">已过期</template><!-- 这里后台新增一个字段2 -->
						</view>
					</view>
